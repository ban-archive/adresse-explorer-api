...
async function doImport({banPath, banoPath, cadastrePath, importInMongo, writeAddokFile}) {
  await mongo.connect()
  ...
  if (writeAddokFile) {
    consolidatedVoies.forEach(voie => {
      const addokVoie = convertToAddok(voie)
      console.log(JSON.stringify(addokVoie))
    })
