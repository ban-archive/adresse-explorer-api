...
const {intersection, union, pick, mapValues, first, groupBy} = require('lodash')
...
async function doImport({banPath, banoPath, cadastrePath}) {
  ...
  consolidatedVoies.forEach(voie => {
    ...
    numeros.forEach(n => {
      if (n.destination) {
        n.destination.forEach(d => destinations.add(d))
      }
    })
    voie.destinations = [...destinations]
    voie.active = intersection(voie.destinations, ACTIVE_DESTINATIONS).length > 0
