...
const {arrayToStream, ensureParentDirectoryExists} = require('./utils')
...
async function doImport({banPath, banoPath, cadastrePath, importInMongo, addokFilePath}) {
  await mongo.connect()
  ...
  if (addokFilePath) {
    await ensureParentDirectoryExists(addokFilePath)
    await pipe(
      arrayToStream(consolidatedVoies.map(convertToAddok)),
      stringify(),
      createGzip(),
      createWriteStream(addokFilePath)
    )
