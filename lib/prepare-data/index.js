...
async function prepareData({banSourcePattern, banoSourcePattern, cadastreSourcePattern, ftthSourcePattern, departements, addokFilePattern}) {
  ...
  await Promise.all(departements.map(async departement => {
    ...
    await runWorker({banPath, banoPath, cadastrePath, ftthPath, addokFilePath, departement})
  }))
